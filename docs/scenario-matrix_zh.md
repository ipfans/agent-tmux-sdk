...
| HAR-01 | 已完成 | `test/fake-harness.test.ts` | 假 tmux/假 Claude 测试工具不存在。 | 确定性的假 `createSession`、`killSession`、`startClaude`、`exitClaude`、`resumeClaude`、`execute`、`capturePane`、`switchAccount`、故障标志、Token 耗尽恢复流程、Claude 会话 ID 跟踪。 | `pnpm test` |
...
| ACCT-01 | 已完成 | `test/account-switching.test.ts` | 账户切换不存在。 | 空闲进程立即切换、繁忙进程延迟切换、相同账户空操作、失败包装。 | `pnpm test` |
