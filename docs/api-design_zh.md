...
`agent-tmux-sdk` 管理一组运行在长生命周期 tmux 会话中的 Claude CLI 进程池。核心模型：**tmux 会话 = 容器（长生命周期）**，**Claude 进程 = 消费者（tmux 内的短生命周期进程）**。SDK 负责 tmux 容器生命周期、Claude 消费者生命周期、任务分配、空闲重启、账户切换、Token 耗尽恢复和两阶段清理。
...
| `account` | `undefined` | 传递给消费者启动的初始 Claude 账户/配置标签。 |
...
- `runTask<TResult>(options)` — 返回 `TaskResult<TResult>`，包含 `taskId`、`state`、`output`、可选的解析后 `result`、生命周期元数据（`startedAt`、`completedAt`、`resumed`）以及调用方提供的 `metadata`。
...
- **账户切换**：`switchAccount(account)` 立即切换空闲进程的账户，繁忙进程则推迟到下次进入空闲状态时切换。相同账户的切换为空操作。
- **清理**：两阶段拆解。首先，`exitClaude` 优雅退出每个 Claude 消费者。然后，`killSession` 销毁 tmux 容器。该操作是幂等的。清理后拒绝新任务。
...
├── TmuxError          — tmux 容器/Claude 消费者启动、重启、账户切换、清理失败
...
- `switchAccount(sessionName, account)` — 为 Claude 消费者切换账户/配置。
...
| `readyPattern` | `/✻ (Baked\|Took\|Done\|Cogitated)/` | 用于检测 Claude 响应完成后就绪状态的正则表达式。 |
...
完整的测试契约覆盖面（14 个文件、50 个测试）请参见 [scenario-matrix_zh.md](scenario-matrix_zh.md)。
