...
| STREAM-01 | Complete | `test/streaming.test.ts` | Streaming did not exist. | `runStream` yields adapter chunks in order with `streamChunk`/`taskStarted`/`taskCompleted` events, accumulates output, returns the slot to idle on success/error, honors `timeoutMs` (typed timeout), interrupts the slot on early break/error (not on clean completion), and rejects after cleanup. | `pnpm test` |
...
- `pnpm test`: 18 files, 134 tests passed (fast, fake-only; integration excluded).
